...
pixel_per_inch_circle = 1584.3953488372092
threshold_value = 30
...
while True:
    ...
    read_m15 = modbus_tcp.client.read_coils(8207, 1)  # M15 - Trạng thái chuẩn bị chụp chp Capture 2/2
    m15_status = read_m15.bits[0]
    ################## XỬ LÝ ẢNH VÀ UPDATE GIAO DIỆN ###################
    grab_result = camera.RetrieveResult(5000, pylon.TimeoutHandling_ThrowException)
    if grab_result.GrabSucceeded():
        img = converter.Convert(grab_result).GetArray()
        if m18_status and image_counter == 0:
        ...
        if image_counter > 1:  # Trường hợp bắt được 2 lỗi trên 1 mặt gỗ
            image_counter = 1
        if m15_status and image_counter == 1:
            output_dir = f'E:\CXHCTL_Phuc\Programs\PythonCode\pythonProject\stitching_images'
            if not os.path.exists(output_dir):
                os.makedirs(output_dir)
            ...
            modbus_tcp.client.write_coil(8212, 1)  # M20 - Biến tạm của Capture 2/2
            stitch_and_save_images()
            image_counter = 0
