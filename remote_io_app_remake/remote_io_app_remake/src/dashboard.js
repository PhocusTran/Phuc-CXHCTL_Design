...
function generateButtonGrid(type, container, groundId, totalButtons, columns = 4) {
    ...
    for (let i = 0; i < totalButtons; i += columns) {
        ...
        for (let j = 0; j < columns && (i + j) < totalButtons; j++) {
            ...
            element.setAttribute("id", `${groundId}${i+j}`);
            element.value = 0;
...
function generateGrid(deviceId, ins, outs){
    ...
    generateButtonGrid('span', inputGrid,`${deviceId}/In/`, ins);
    ...
    generateButtonGrid('button', outputGrid,`${deviceId}/Out/`, outs);
...
function change_pin_state(id, str, tag){
    substrs = document.getElementById(id).value.split(":");
    max = parseInt(substrs[1]);
    mark = parseInt(str)
    for(i=0;i<max;i++){
        element = document.getElementById(`${id}/${tag}/${i}`)
        if((1 << i) & mark){
...
wsClient.addEventListener('onmessage', (event) => {
    try {
        ...
        if (data.IF){
            substrs = data.IF.split("/");
            change_pin_state(substrs[0], substrs[1], "In");
        }

        if (data.OF){
            substrs = data.OF.split("/");
            change_pin_state(substrs[0], substrs[1], "Out");
        }
...
function output_button_click(event){
    ...
    var bf = JSON.stringify({
        key: "TO",
        value: `${button.id.split("/")[0]}/${1 << parseInt(button.id.slice(-1))}`,
    })
