...
def findrect(img):
    ...
    contours = cv2.findContours(dilated, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)[0]  # Get contourns cv2
    ...
    for cnt in contours:
        ...
        if len(approx) == 4 and 88 < angle(approx[1], approx[0], approx[2]) < 92 and 88 < angle(approx[3], approx[0], approx[2]) < 92:  # if cnt can be approx with only 4 points, it is a 4 side polygon
            ...
            if w * h > 1000:  # Clean very small rectangles
                ...
                class_list.append(
                    rect('rect_' + str(r), x, y, w, h, crop_img, 'green'))  # Get a list with the rect class
                r = r + 1
