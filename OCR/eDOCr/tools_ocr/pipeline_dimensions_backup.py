...
class Pipeline:
    ...
    def recognize_dimensions(self, box_groups, img):
        ...
        i = 0
        recognition_kwargs = {}
        box_groups = sorted(box_groups, key=lambda box: (np.mean(box[:, 0])))
        for box in box_groups:
            ...
            if len(cnts) == 1:
                ...
                self.recognizer.recognize_from_boxes(images=[img_croped], box_groups=box_groups, **recognition_kwargs)[
                    0][0]
                pred, add = analyse_pred(pred, cnts, id=i + 1)
            elif 1 < len(cnts) < 15:
...
def analyse_pred(pred,cnts,id=None):
    ...
    if any(char.isdigit() for char in pred):
        ...
        if 'Ra' in pred:
            ...
            if ' ' in pred:
                ...
                print(f"ID: {id}, Nominal: {pred}, Value: {split_pred[0]}, Tolerance: ±{split_pred[1]}")
            else:
                pred_dict.update(nominal=pred, value=re.sub("[\(].*?[\)]", "", pred), tolerance='general')
    else:
        add=False
